...
CONFIG_REL="configs/pilot_web_500step_formal_schedule.toml"
OUTPUT_REL="outputs/m3_web_global77_formalschedule_500step_pilot_seed11"
...
if [[ ! -f "${CONFIG_REL}" || ! -f "${TRAIN_MANIFEST_REL}" || ! -f "${VAL_MANIFEST_REL}" ]]; then
  echo "Missing the 500-step pilot config or canonical manifests." >&2
  exit 1
fi
