...
on_error() {
  local status=$?
  echo "SAT F1/F2 mechanism screen stopped at line ${BASH_LINENO[0]} (exit=${status})." >&2
  echo "Completed boundaries are reusable; rerun the same command after fixing the cause." >&2
  exit "${status}"
}
trap on_error ERR
...
usage() {
  cat <<'EOF'
Usage: bash scripts/run_skyscript_m4_f1_f2_sat_seed11.sh [options]

Options:
  --rsicd-root PATH  RSICD root (default: assets/data/raw/rsicd)
  --batch-size N     Evaluation forward batch size (default: 64)
  --num-workers N    Evaluation workers (default: 4)
  --chunk-size N     Retrieval score chunk size (default: 256)
  -h, --help         Show this help.

Runs four SAT-only seed11 candidates through 500 steps and writes one summary.
The archived M4-A adapter-only SAT result is reused as F0 and is never retrained.
The SAT visual backbone remains frozen in every candidate. Run inside tmux.
EOF
}
...
STUDY_DIR="outputs/skyscript_m4_f1_f2_sat_seed11"
...
declare -A CONFIGS=(
  [f1_visionhead_lr1e5]="configs/skyscript_sat_f1_visionhead_lr1e5_500step_seed11.toml"
  [f1_visionhead_lr5e6]="configs/skyscript_sat_f1_visionhead_lr5e6_500step_seed11.toml"
  [f2_adapter256_visionhead_lr1e5]="configs/skyscript_sat_f2_adapter256_visionhead_lr1e5_500step_seed11.toml"
  [f2_adapter256_visionhead_lr5e6]="configs/skyscript_sat_f2_adapter256_visionhead_lr5e6_500step_seed11.toml"
)
...
python tools/verify_skyscript_m4_f1_f2_configs.py
...
verify_candidate() {
  ...
  if [[ ! -e "${report}" ]]; then
    ...
    if [[ "${candidate}" == f1_* ]]; then
      args+=(--required-optimizer-group vision_head)
    else
      args+=(--required-optimizer-group image_adapter --required-optimizer-group vision_head)
    fi
    local checkpoint
...
for candidate in "${CANDIDATES[@]}"; do
  echo "===== SAT mechanism candidate: ${candidate} ====="
  train_candidate "${candidate}"
  verify_candidate "${candidate}"
  evaluate_candidate "${candidate}"
done

python tools/summarize_skyscript_m4_f1_f2_sat.py \
  --baseline "${F0_SUMMARY}" \
  --candidate-root outputs \
  --output-root outputs \
  --output "${STUDY_DIR}/summary.json"

echo "SAT F1/F2 mechanism screen complete."
