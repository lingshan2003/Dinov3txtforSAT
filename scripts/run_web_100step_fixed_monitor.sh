...
CONFIG_REL="configs/verify_web_100step_fixed_monitor.toml"
OUTPUT_REL="outputs/m3_web_global77_100step_fixedmonitor_seed11"
...
if [[ ! -f "${CONFIG_REL}" || ! -f "${TRAIN_MANIFEST_REL}" ]]; then
  echo "Missing the fixed-monitor config or canonical training manifest." >&2
  exit 1
fi
...
python tools/verify_training_run.py \
  ...
  --fixed-monitor-every 10 | tee "${OUTPUT_REL}/verification_report.json"
