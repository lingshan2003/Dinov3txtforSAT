...
CONFIG_REL="configs/verify_web_100step_validation_resume.toml"
OUTPUT_REL="outputs/m3_web_global77_100step_validation_resume_seed11"
...
if [[ ! -f "${CONFIG_REL}" || ! -f "${TRAIN_MANIFEST_REL}" || ! -f "${VAL_MANIFEST_REL}" ]]; then
  echo "Missing the validation/resume config or canonical manifests." >&2
  exit 1
fi
